...
class TestQueenController(unittest.TestCase):
    def setUp(self):
        self.context = DroneContext()
        self.context.ticks_since_inventory = 0
        self.controller = QueenController(self.context)
    ...
    def test_tick_triggers_inventory_refresh(self):
        # Set ticks to trigger inventory refresh (15)
        self.context.total_ticks = 14
        action = self.controller.tick()
        self.assertEqual(action, "Inventory")
        self.assertEqual(self.context.total_ticks, 15)
