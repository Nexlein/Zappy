...
// stu x (this is a custom event, not part of the official protocol, representing the server uptime in seconds)
struct ServerUptime {
    int uptimeSeconds;
};

// sse #e N X Y (this is a custom event, not part of the official protocol, representing an egg spawned by the server)
...
using Event = std::variant<MapSize, TileContent, TeamName, PlayerNew, PlayerPosition, PlayerLevel,
                           ...
                           GameEnd, ServerMessage, UnknownCommand, BadParameters, ServerUptime, ServerSpawnedEgg>;
