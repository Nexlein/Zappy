...
void clap::App::print_help()
{
    ...
    for (const auto& pos : _positionals) std::cout << " <" << pos->names() << ">";
    std::cout << "\n\n" << description << "\n";

    const std::string help_prefix = "  -h,--help";
    size_t max_w = help_prefix.size();
    for (const auto& arg : _arguments) max_w = std::max(max_w, arg->prefix().size());
    for (const auto& pos : _positionals) max_w = std::max(max_w, pos->prefix().size());
    ...
    std::cout << std::left << std::setw(col) << help_prefix << "Show this help message\n";

    if (!_positionals.empty()) {
        std::cout << "\nArguments:\n";
        for (const auto& pos : _positionals)
            std::cout << std::left << std::setw(col) << pos->prefix() << pos->description() << "\n";
    }
}
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (starts_with(token, "--")) {
            ...
            if (eq != std::string_view::npos) {
                ...
                if (!arg) throw clap::UnknownArgument(std::string(arg_name));
                if (!arg->takes_value())
                    throw clap::ParseError("flag '" + std::string(arg_name) +
        ...
        if (!starts_with(token, "--") && token.size() > 2) {
            for (size_t j = 1; j < token.size(); ++j) {
                ...
                if (!arg) throw clap::UnknownArgument(short_name);
                if (arg->takes_value())
        ...
        if (!arg) throw clap::UnknownArgument(std::string(token));

        if (arg->takes_multiple_values()) {
            if (i + 1 >= argc || starts_with(argv[i + 1], "-"))
                throw clap::MissingValue(std::string(token));
            while (i + 1 < argc && !starts_with(argv[i + 1], "-")) arg->push_value(argv[++i]);
        } else if (arg->takes_value()) {
            if (i + 1 >= argc) throw clap::MissingValue(std::string(token));
            arg->parse(argv[++i]);
        } else {
            arg->parse("");
        }
